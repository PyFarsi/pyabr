    ...
    def on_clicked(self, index):
        ...
        if x == True:
            if files.isfile(f'/usr/share/locales/{self.item.whatsThis()}.locale'):
                files.write('/proc/info/lsel',self.item.whatsThis())

# select box #
class MainApp (QMainWindow):
    def onCloseProcess (self):
        if not app.check('lang'):
            self.Widget.Close()
        else:
    ...
    def inp(self):
        control.write_record('locale',files.readall('/proc/info/lsel'),'/etc/gui')
        control.write_record('layout',files.readall('/proc/info/lsel'),'/etc/gui')
        control.write_record('font', control.read_record("font",f"/usr/share/locales/{files.readall('/proc/info/lsel')}.locale"), '/etc/gui')
        control.write_record('submenu.direction', control.read_record("submenu-direction",f"/usr/share/locales/{files.readall('/proc/info/lsel')}.locale"), '/etc/gui')
