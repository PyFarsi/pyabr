...
f = QFont(getdata("font"),int(getdata("fontsize")))
...
class FileListView(QListView):
    ...
    def mkdir(self, dirname):
        if files.isfile(dirname):
            ...
            try:
                commands.mkc([str(f"{self.dir}/{dirname}").replace(f'/stor/{files.readall("/proc/info/mnt")}/', '')])
            except:
    ...
    def mkfile (self,filename):
        if files.isdir(filename):
            ...
            it.setWhatsThis(f"{self.dir}/{filename}")
    ...
    def genpa (self,filename):
        it = QtGui.QStandardItem(f"{filename}.pa")
        it.setWhatsThis(f"{self.dir}/{filename}.pa")
        self.entry.appendRow(it)
        self.format(it, f"{filename}.pa")
        it.setFont(self.editor.Env.font())
    ...
    def mkcode (self,filename):
        ...
        if files.isdir( f"{filename}.{self.ext}"):
            self.editor.Env.RunApp('text', [res.get('@string/isdir'),res.get('@string/isdir_msg').replace('{0}',f"{filename}.{self.ext}")])
            app.switch('persia')
        else:
            self.mkfile(f"{filename}.{self.ext}")
            files.write( f'{self.dir}/{filename}.{self.ext}',files.readall(res.get(control.read_record('connect',res.get(self.code)))))
...
class MainApp(QtWidgets.QMainWindow):
    ...
    def project_create (self,projectname):
        try:
            ...
            control.write_record('project_pack',  f'{projectname}.pa', 'config')
    ...
    def generate_pa_ (self):
        try:
            ...
            self.config =  f"{self.path}/config"
    ...
    def New_Code (self):
        files.write('/tmp/code.tmp',f'@temp/{self.sender().objectName()}')
        app.switch('roller')
